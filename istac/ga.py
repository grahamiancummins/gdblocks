...
class AcellCross(Model):
	...
	def eval(self, pars):
		...
		try:
			...
			self.cm(self.doc,  **{'evts1':'evt1','bootstrap':20, 'evts2':'evt2', 'stim':self.snn, 'outpath':self.snn})
			f = -1*self.doc['stimulus']['fitness']
			if f <= 1:
				return (f, ())
			else:
				self.cm(self.doc,  **{'compress':self.compress, 'clevel':self.clevel, 'evts1':'evt1', 'evts2':'evt2', 'bootstrap':20, 'stim':self.snn, 'outpath':self.snn})
