...
def parseStim(l):
    ...
    if stim['type'] == 'stored file':
        ...
        print(
        "This stimulus type (%s) isn't supported. Instead setting the 'value' attribute to the raw contents of the stimulus line" %
        stim['type'])
        stim['value'] = l
    return stim
...
def getTest(sections):
    ...
    t = te['lines']
    l3 = t[2].split()
    test = {'testtype': t[0], 'time': t[1], 'size': int(l3[0]), 'scan': ' '.join(l3[1:]), 'traces': [],
            'comment': te['comment']}
