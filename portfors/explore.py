...
def traces(d, ns, st, shift=True):
	...
	for tk in d[ns]:
		if tk.startswith('trace'):
			trn = int(tk[5:])
			tra = d[ns][tk]
			stim = tra[st] or {'type':'No Stimulus', 'value':'No Stimulus', 'delay':0.0, 'attenuation':1000.0, 'duration':0}
			trd = {}
			desc = _descstim(stim)
			if shift:
...
def getcell(cellid, dname='bicIC', sheet='fulldatasheet.ods', tab='sheet1.table', cellidlab='cell', 
	...
	clines = [(ds[i, mousecol], ds[i,testcol], ds[i, condcol]) for i in range(ds.shape[0]) if ds[i,cellcol]==str(cellid)]
	if not clines:
...
def manyrasters(d, maxrows=4, maxcols=3, vsplit='condition', hsplit='cell', csplit='stim_type', sort=('stim_atten',), lab='calls'):
	f=plt.figure(1)
	f.set_figwidth(10)
	f.set_figheight(8)
	vkeys = allv(d, vsplit)
