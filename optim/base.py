	...
	def prep(self):
		...
		if self.nunits:
			...
			if snd < self.range.nd:
				raise StandardError('Attempt to resume %i-d optimizer from a storage of %i-d parameters' % (self.range.nd, snd))
			elif snd > self.range.nd:
				report('resuming an %i-d optimizer from a store of %i-d parameter sets. Hopefully this is intentional (eg. there are eval-conditions in the model). If not, dont run this optimizer' % (self.range.nd, snd))
	...
	def insert(self, fit, ps):
		...
		if fit > self.best[0]:
			...
			if self.model.perfect!=None and fit == self.model.perfect:
				report("You hear the singing of Angels and a chorus of trumpets, hailing the birth of a perfect unit. All further competition is futile. Exiting")
